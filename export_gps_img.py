...
def collect_points(folder: Path, recursive: bool) -> Tuple[List[PhotoPoint], int]:
    ...
    #points.sort(key=lambda p: (p.dt, p.path.name.lower()))


    """with open('flist.txt', 'w+') as f:
        f.write("\nImages in chronological order:\n")
        for i, p in enumerate(points, start=1):
            f.write(f"{i:04d}  {p.dt.isoformat(sep=' ')}  {p.path}\n")"""
...
def render_map_image(
    ...
    """
    Generates a static PNG map using OSM tiles.

    width_px/height_px let you make portrait or landscape maps.
    """
    m = StaticMap(width_px, height_px, url_template="https://a.tile.openstreetmap.org/{z}/{x}/{y}.png")

    # Tour polyline (optional)
    if draw_line and tour_coords and len(tour_coords) >= 2:
        line = Line([(lon, lat) for lat, lon in tour_coords], "#1f77b4", 3)  # blue
        m.add_line(line)

    # Marker for this photo
    lat, lon = point_coords
    marker = CircleMarker((lon, lat), "#d62728", 12)  # red
    m.add_marker(marker)
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Export one static map PNG per GPS-tagged photo, highlighting where it was taken."
    )
    ap.add_argument("folder", nargs="?", default=".", help="Folder to scan (default: .)")
    ...
    # NEW: configurable aspect ratio
    ap.add_argument("--width", type=int, default=700, help="Map image width in px (e.g., 700)")
    ap.add_argument("--height", type=int, default=1000, help="Map image height in px (e.g., 1000 for portrait)")

    ap.add_argument("--zoom", type=int, default=12, help="Map zoom level (typical: 10-15)")
    ap.add_argument(
        "--line",
        choices=["none", "full", "upto"],
        default="upto",
        help="Draw tour line: none | full (entire tour) | upto (tour up to that photo)",
    )
    ap.add_argument(
        "--center",
        choices=["photo", "tour"],
        default="photo",
        help="Map centering: photo (center on this photo) | tour (fixed center of entire tour)",
    )
    args = ap.parse_args()
    ...
    points, skipped = collect_points(folder, args.recursive)

    print(f"Scanned: {folder}")
    print(f"GPS+datetime photos: {len(points)}")
    print(f"Skipped (no GPS/dt/unreadable): {skipped}")

    if not points:
        print("No GPS+datetime photos found.")
        return 0

    all_coords = [(p.lat, p.lon) for p in points]
    ...
    draw_line = args.line != "none"

    for i, p in enumerate(points):
        if args.line == "full":
            tour_coords = all_coords
        elif args.line == "upto":
            tour_coords = all_coords[: i + 1]
        else:
            tour_coords = None
        ...
        if (i + 1) % 25 == 0 or (i + 1) == len(points):
            print(f"Rendered {i+1}/{len(points)}")
